...
class AddReply extends GetxController {
  final dioConfig = CommonDioConfiguration();

  Future<Map<dynamic, dynamic>?> addReplyApi(String content, video_id, comments_id) async {

    try {
      final response = await dioConfig.dio.post(
          '${Environment.host}/posts/$video_id/comments/$comments_id/',
          data: {
            "content": content
          }
      );
